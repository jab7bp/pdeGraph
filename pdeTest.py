...
class PhDAnalyzier():
    ...
    def CalcNpe(self):  # calculate average # of detected photons per pulse
        ...
        ####Notice: there are redundant values here for calculation verification purposes
        ####I am duplicating them in order to not change the use/ordering of the original values/variables
        ####Redundant values are: A0, noise0, A1, noise1
        ####Associated with these are A_error, Noise_err, A1Err, noise1Err
        ...
        if self.hPhD0:
            ...
            ###Determine Dark Pedestal and total Dark Counts
            self.nDarkPed=A/self.hPhD0.GetBinWidth(1) * sqrt(2*pi) * noise
            self.nDarkTot=self.hPhD0.Integral(1,self.hPhD0.GetNbinsX())
        
            ####Breaking up this portion of the NPE calculation: NPE = -log(A*N) + log(A1*N1)
        ...
        ###Variables for GetEntries, BinWidth, and Integral are wrapped into AN and A0N0
        ###For "manual" calculation, NPE and NPE Uncertainty is calculated and sent as: NPE = -log(AN) + log(A1N1)
        ...
        ###Save NPE and associated error into the NPE array
        ...
        ###Store the fit parameters A0, N0, A1 and N1
        A0_w_err.append(A0)
        ...
        ####Make these variables global so that they can be retrieved throughout this code and sent across programs via the definitions at the bottom of the code
        global A0_w_err
        global n0_w_err
        global A1_w_err
        global n1_w_err
...
####Various definitions for retrieving various values from the code up above.
####Used for pulling values into other codes
    def GetNoise(self):
        noise_w_err = []
